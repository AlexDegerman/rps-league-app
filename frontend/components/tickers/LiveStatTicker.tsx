...
export default function LiveStatsTicker() {
  ...
  if (!stats) return null
    ...
    return (
      <div
        className={`rounded-b-xl py-1.5 px-3 flex items-center gap-2 shadow-sm relative z-0 overflow-hidden
      bg-gray-50/80 border-x border-b border-gray-100
      ${modeKey ? `event-bg-${modeKey} event-side-${modeKey}` : ''} 
    `}
      >
        {/* Background/Particle layer to match header depth */}
        {modeKey && (
          ...
          <div
            ...
            style={{
              background: getEventColor(
                modeKey.replace(/^(flash_|festival_|winstreak_)/, ''),
                0.6
              ),
              boxShadow: `0 0 6px ${getEventColor(modeKey.replace(/^(flash_|festival_|winstreak_)/, ''), 0.4)}`
            }}
          />
        )}
