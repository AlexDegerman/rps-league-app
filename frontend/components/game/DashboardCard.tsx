...
export default function DashboardCard() {
  ...
  return (
    <div
      className={`rounded-xl border shadow-sm p-2 transition-all duration-500 relative z-10 ${borderClass}`}
    >
      <div className="absolute inset-0 bg-white -z-20 rounded-xl" />
      {modeKey && (
        <>
          <div
            className={`absolute inset-0 -z-10 event-bg-${modeKey} event-side-${modeKey} rounded-xl`}
          />
          <div className="absolute inset-0 -z-10 rounded-xl overflow-hidden pointer-events-none">
            <div
              className={`event-dynamic-particles particles-${modeKey} absolute inset-0`}
            />
          </div>
        </>
      )}
