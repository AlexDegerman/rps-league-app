...
function BottomConfetti({
  ...
  // Flash types have their own top-of-card confetti handled by ConfettiOverlay
          ...
          return (
            ...
            >
              <div className="flex flex-col items-center px-4 text-center">
                <div
                  className={`badge-aura-wrapper ${bonusTierStyle.auraClass} inline-flex items-center mb-1`}
                >
                  <span
                    className={`text-[10px] px-2.5 py-0.5 font-black uppercase tracking-widest rounded-full border relative z-10 ${bonusTierKey === 'LEGENDARY' ? 'text-yellow-700 border-yellow-500 bg-yellow-50' : `${bonusTierStyle.color} ${bonusTierStyle.bg} border-black/5`}`}
                  >
                    {visual.label}
                  </span>
                </div>
                {!resultAnim.win && (
                  <div
                    className={`text-[14px] font-black uppercase italic tracking-tighter mb-1 leading-none ${bonusTierStyle.color}`}
                  >
                    Lucky Save
                  </div>
                )}
                ...
                >
                  <span
                    className={`text-2xl sm:text-3xl font-black tabular-nums tracking-tighter ${bonusTierStyle.amountColor}`}
                  >
                    {resultAnim.win ? '+' : ''}
                    <span className={getAmountColor(resultAnim.bonus.amount)}>
