...
- **Popup Queue Hook**: Verifies delayed popup processing, duplicate prevention, timer cleanup on unmount, bonus stage interruption handling, flash event activation, global event synchronization, achievement batching, expired event removal, and audio routing for popup-specific themes.
- **Relic System Components**: Verifies empty slot states and capacity indicator accuracy, drawer opening triggers, charge counter progression on equipped relics, readonly state enforcement, discovery drop popup rendering and rarity audio dispatching, immediate auto-equipping into available preset slots, replacement slot selection when loadouts are full, popup dismissal queue handling, drawer visibility toggling, 3-preset tab switching with active in-game indicators, category-specific catalog filtering, one-click auto-equipping from inventory, and combat lock restrictions during active World Boss encounters.